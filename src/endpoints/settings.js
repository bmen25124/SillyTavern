...
router.post('/get', jsonParser, (request, response) => {
    ...
    const { fileContents: api_presets, fileNames: api_preset_names }
       = readPresetsFromDirectory(request.user.directories.api, {
        sortFunction: sortByName(request.user.directories.api), removeFileExtension: true,
    });
